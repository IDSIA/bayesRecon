...
When you click the **Knit** button a document will be generated that includes both content as well as the output of any embedded R code chunks within the document. You can embed an R code chunk like th---
...
In this example, we assume that the base forecasts are Poisson distributions, 
which is a common choice for count time series.
We first generate the series, then we compute the base forecasts, 
and finally we reconcile them using `reconc_BUIS`.

To generate the series, we randomly generate the bottom series 
and then aggregate them using **A** to obtain the upper series.
...
We then compute the reconciled forecasts using the BUIS algorithm 
...
We compare the base and reconciled forecasts for the top series by plotting the base and reconciled forecast distributions.
...
however, we assume that the base forecasts are multivariate Gaussian, 
which is a common choice for real-valued time series.
The covariance matrix of the base forecasts is estimated from the in-sample residuals of the fitted models.

First, we generate the series using an AR(1) process for the bottom series, 
and then we aggregate them to obtain the upper series.
...
We then analytically compute the reconciled forecasts via conditioning using the `reconc_gaussian` and the `reconc_t` functions.
...
The `reconc_t` method adopts a Bayesian approach to account for the uncertainty of the covariance matrix of the base forecasts;
...
Finally, we compare the two reconciled forecast distributions for the top series T by plotting their marginal densities.
...
As before, we randomly generate the bottom series 
and then aggregate them using **A** to obtain the upper series.
...
We compute the *base forecasts* for the bottom series using the package [`glarma`](https://cran.r-project.org/package=glarma).
...
and `reconc_TDcond` for large hierarchies (see [Zambon et al. 2024](https://proceedings.mlr.press/v244/zambon24a.html) for details).
...
forecast distributions as PMFs.
...
df_bars <- data.frame(
  x    = rep(x_int, 2),
  prob = c(pmf_mc[x_int+1], pmf_td[x_int+1]),
...
ggplot() +
  ...
  geom_line(
    data = df_line,
    aes(x = x, y = dens, color = type, linetype = type),
    linewidth = 1.3
  ) +
  scale_fill_manual(
  ...
  scale_color_manual(
    values = c("Base (Gaussian)" = "#619CFF"),
    name = NULL
  ) +
  scale_linetype_manual(
    values = c("Base (Gaussian)" = "solid"),
    name = NULL
  ) +
  labs(
