...
.getAfromS <- function(S) {
  ...
  A = S[upper_idxs,]
  out = list(A=A, upper_idxs=upper_idxs, bottom_idxs=bottom_idxs)
  return(out)
}
...
.split_hierarchy <- function(S, Y) {
  ...
  out = list(A=getAfromS.res$A,
             upper=upper, bottom=bottom,
             upper_idxs=getAfromS.res$upper_idxs, bottom_idxs=getAfromS.res$bottom_idxs)
  return(out)
}
