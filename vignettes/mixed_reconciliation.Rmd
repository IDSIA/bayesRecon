...
base_forecasts_mu <- c(mu_u, mu_b)
base_forecasts_Sigma <- matrix(0, nrow = n, ncol = n)
base_forecasts_Sigma[1:n_u, 1:n_u] <- Sigma_u
base_forecasts_Sigma[(n_u + 1):n, (n_u + 1):n] <- Sigma_b
```
...
# Gaussian reconciliation
start <- Sys.time()
gauss <- reconc_gaussian(A, base_forecasts_mu, base_forecasts_Sigma)
stop <- Sys.time()

rec_fc$Gauss <- list(
  mu_b = gauss$bottom_reconciled_mean,
