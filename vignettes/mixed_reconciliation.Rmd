...
Sec. 5 of the paper presents the results of the experiments conducted on 10 different stores. 
...
with the function `bayesRecon::reconc_gaussian`. We refer to this method as `Gauss`.

The upper base forecasts are assumed to be a multivariate Gaussian, whose covariance matrix is estimated from the 
in-sample residuals, while the bottom base forecasts are assumed to be independent.
...
The algorithm is implemented in the function \link{reconc_MixCond}. 
...
This algorithm is implemented in the function \link{reconc_TDcond}. 
...
The algorithm TD-cond still returns a warning regarding the high incoherence between the joint bottom-up and the upper base forecasts. 
...
IS_pmf <- function(pmf, actual, alpha) {
...
mis$base[(n_u+1):n]  <- mapply(IS_pmf, pmfs, actuals_b, MoreArgs = list(alpha=alpha))
...
mis$MixCond  <- mapply(IS_pmf, pmfs, actuals, MoreArgs = list(alpha=alpha))
...
mis$TDcond  <- mapply(IS_pmf, pmfs, actuals, MoreArgs = list(alpha=alpha))
...
boxplot(skill_scores$mase$upper, main = "MASE upper time series", 
        col = custom_colors, ylim = c(-200,50))
abline(h=0,lty=3)
...
boxplot(skill_scores$mis$upper, main = "MIS upper time series", 
        col = custom_colors, ylim = c(-150,50))
abline(h=0,lty=3)
...
boxplot(skill_scores$rps$upper, main = "RPS upper time series", 
        col = custom_colors, ylim = c(-120,50))
abline(h=0,lty=3)
