...
We release a new data set, containing time series of conuts of extreme market  events 
in five economic sectors in the period 2005-2018 (3508 trading days).  
The counts are computed by considering 29 companies 
...
We divide the companies in the following  sectors: Financial (FIN), Information and Communication Technology (ICT), 
Manufacturing (MFG), Energy (ENG), and Trade (TRD). 

We thus have five daily time series of counts, corresponding to the number of extreme market  events in each sector.
We build  a hierarchy with 5 bottom time series 
(one for each  sector)
and 1 upper time series (the sum of  the different sectors). 

```{r out.width = '50%', echo = FALSE}
knitr::include_graphics("img/finTS_hier.jpg")
```
We also release  the base forecasts to be reconciled. 
As discussed in the paper, they are  produced
using the model by [@agosto2022multivariate];
the predictive distributions
are  negative binomial.
These are  in-sample base forecasts: for each training instant, they are computed for time t+1 by conditioning on the counts observed up to time t. 

Base forecasts and  actual values, available for 3508 days,  are stored in: `bayesRecon::extreme_market_events`.
We now extract the summing matrix, the aggregating matrix, the base forecasts and the actual values.
...
# # If you want to run only N  reconciliations (instead of 3508), use the following approach:
# N <- 200
# actuals <- actuals[1:N,]
...
We reconcile the base forecasts via conditioning, using
the BUIS algorithm [@zambon2022efficient].
Using the `reconc_BUIS` function, we perform 3508 reconciliations, one for each day, drawing each time 10000 samples  from the reconciled distribution.
...
for (j in 1:N) {
  ...
  buis <- reconc_BUIS(S, base_fc_j, "params", "nbinom", num_samples = 1e4, seed = 42)
...
stop <- Sys.time()
cat("Computational time for ", N, " reconciliations: ", round(stop-start, 2), "s")
```
...
mean_skill_scores <- c(round(colMeans(SS_AE), 2), 
...
We observe two different behaviours for the reconciled upper mean:
the concordant-shift and the combination effects.
...
j <- 124
base_fc_j <- c()   
for (i in 1:n) base_fc_j[[i]] <- c(extreme_market_events$base_fc$mu[[j,i]], 
                                   extreme_market_events$base_fc$size[[i]])
# Reconcile
...
cat("Base upper mean: ",       round(extreme_market_events$base_fc$mu[[j,1]], 2))
cat("Bottom-up upper mean: ",  round(sum(extreme_market_events$base_fc$mu[j,2:n]), 2))
cat("Reconciled upper mean: ", round(mean(samples_y[1,]), 2))
...
base_bottom_var <- mapply(function(mu, size) var(rnbinom(n = 1e5, size = size, mu = mu)),
                          extreme_market_events$base_fc$mu[j,2:n], 
                          extreme_market_events$base_fc$size[2:n])
