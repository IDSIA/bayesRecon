 ...
 * `reconc_gaussian`: reconciliation via conditioning assuming multivariate Gaussian base forecasts; 
    this is done analytically;
 * `reconc_t`: reconciliation via conditioning assuming multivariate Gaussian base forecasts with uncertain covariance matrix; 
...
# Compute the actual value of T at time n_obs+1 
last_B <- as.numeric(B_ts[n_obs, ])
B_actual <- 0.8 * last_B + rnorm(4, sd = 0.5)  # use same AR(1) data generating process
T_actual <- sum(A[1, ] * B_actual)
...
lambdas_new <- lambda_bls + seas_new + rnorm(4, sd = 0.1)
B_actual <- rpois(4, pmax(0, lambdas_new))
T_actual <- sum(B_actual)
